...
def create_credentials_template():
    ...
    template = """# SANATANI'S Free Fire Bot Credentials
# Fill in your Free Fire account credentials below

# Format 1: Comma-separated (RECOMMENDED)
uid=4373120808,password=HYPOEXTERNAL_U46QI_BY_SPIDEERIO_GAMING_YX9Z8

# OR Format 2: Line-separated
# uid: 4373120808
# password: HYPOEXTERNAL_U46QI_BY_SPIDEERIO_GAMING_YX9Z8
...
🎯 Target: {target_uid}
🤖 Bot Name: KRISHNA×CODE
✅ Status: Ready to join

📱 Player will see:
"KRISHNA×CODE wants to join your team"
...
async def generate_guest_accounts(count=1, name="KRISHNA×CODE", password_prefix="FF"):
...
async def generate_and_save_accounts(count, name="KRISHNA×CODE", password_prefix="FF"):
    ...
    try:
        ...
        fields = {
            ...
            2: {
                ...
                6: "iG:[C][B][FF0000] @hypo_.1",  # Nickname
                7: 330,                   # Rank
                8: 1000,                  # Field 8
...
    
[00FF00]ＷＥＬＣＯＭＥ ＴＯ[FF0000] _ _ _ SANATAN'S _ _ _ _   [00FF00]ＢＯＴ
...
[1E90FF]ＤＥＶ - [FF0000]_ _ _ G Y A N   B H A I _ _ _ _  
[FF0000]━[00FF00]━[0000FF]━[FFFF00]━[FF00FF]━[00FFFF]━[FFA500]━[FF1493]━[00FF7F]━[FFD700]━[00CED1]━[9400D3]━[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]
[FF0000]_ _ _  M A N J E E T   B H A I _ _ _ _  
[FF0000]━[00FF00]━[0000FF]━[FFFF00]━[FF00FF]━[00FFFF]━[FFA500]━[FF1493]━[00FF7F]━[FFD700]━[00CED1]━[9400D3]━[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]
[9400D3]M A D E B Y [FF0000] G Y A N   B H A I
[FF0000]━[00FF00]━[0000FF]━[FFFF00]━[FF00FF]━[00FFFF]━[FFA500]━[FF1493]━[00FF7F]━[FFD700]━[00CED1]━[9400D3]━[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]
[FFD700] ＦＯＬＬＯＷ    ＭＥ   ＩＮ   [87CEEB]TELEGRAM: [FF0000] hypo_.1
[FF0000]━[00FF00]━[0000FF]━[FFFF00]━[FF00FF]━[00FFFF]━[FFA500]━[FF1493]━[00FF7F]━[FFD700]━[00CED1]━[9400D3]━[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]"""
    fields = {
    1: 5,
    2: {
...
async def convert_kyro_to_your_system(target_uid, chat_id, key, iv, nickname="SANATANI", title_id=None):
...
async def send_sticker(target_uid, chat_id, key, iv, nickname="SANATANI"):
...
async def send_kyro_title_adapted(chat_id, key, iv, target_uid, nickname="SANATANI"):
...
"""
            ...
            # For example:
            title_packet = await convert_kyro_to_your_system(uid, chat_id, key, iv, nickname="KRISHNA×CODE", title_id=title_id)
...
async def noob(target_uid, chat_id, key, iv, nickname="KRISHNA×CODE", title_id=None):
    """EXACT conversion with customizable title ID"""
    try:
        # Use provided title_id or get random one
        if title_id is None:
...
async def send_all_titles_sequentiallly(uid, chat_id, key, iv, region, chat_type):
    ...
    # Send initial message
    start_msg = f"""[B][C][00FF00] Noobde MANJEET BHAI ya mere ko, agar tu noob bolra toh tu gay hai
...
"""
    ...
    try:
        for index, title_id in enumerate(all_titles):
            ...
            # You'll need to use your existing title sending logic here
            # For example:
            title_packet = await noob(uid, chat_id, key, iv, nickname="KRISHNA×CODE", title_id=title_id)
              ...
              # =================== EMOTE DETECTION ONLY ===================
                    ...
                    try:
                        ...
                        if True:
                            ...
                            try:
                                ...
                                bot_uid = 14010319252
                                message_text = f" Can't accept Your request Talk to GYAN BHAI"
                ...
                if insquad == True:
                    try:
                        ...
                        message = """[B][C][FF0000]█▓▒░░ WELCOME TO SANATAN'S BOT ░░▒▓█
[00FF00]»»————-　★　————-««
[FFFFFF]⚡ BOT POWERED BY:[FF0000] HYPOEXTERNAL 
[FFFFFF]🎯 STATUS: [00FF00]ACTIVE 24/7
[FFFFFF]📱 OWNER: [FFFF00] MANJEET BHAI 
[00FF00]»»————-　★　————-««
...
📝 Examples:
/bio Hello World!
/bio 🤖 Bot by KRISHNA×CODE
/bio Level 70 | Pro Player
/bio Add me: KRISHNA×CODE
...
"""
                        ...
                        if inPuTMsG.strip().startswith('/wlremove'):
                            ...
                            # Check owner
                            if str(response.Data.uid) != "829334134":
...
"""
                            ...
                            if str(response.Data.uid) != "829334134":  # Replace with your actual UID
...
"""
                        ...
                        if inPuTMsG.strip() == '/wllist':
                            ...
                            # Check if owner
                            if str(response.Data.uid) != "829334134":  # Your UID
...
👥 Whitelisted UIDs:"""
                            ...
                            for uid in WHITELISTED_UIDS:
                                if uid != "4373120808":  # Skip owner since already shown
                                    whitelist_msg += f"\n• {uid}"
                                    count += 1
                                    if count >= 20:
...
"""
                        ...
                        if inPuTMsG.strip() == "/admin":
                            ...
                            admin_message = """
[B][C][FFC0CB] DON'T FUÇK THIS BOT WITH SPAMMING COMMAND'S.
USE THE BOT POLITELY.


[b][i][FFC0CB] DEVELOPER: GYAN BHAI[/b]
 
[C][B][0000FF] OWNER: MANJEET BHAI 
...
"""
                        ...
                        BLOCKED_NAMES = ["arbaz", "prakash", "manjeet", "gyan"]  # Add your actual name
...
[9400D3]M A D E B Y [FF0000] G Y A N    B H A I
[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]
[FFD700] O W N E R : [FF0000]M A N J E E T B H A I
[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]
...
async def MaiiiinE():
    ...
    os.system('clear')
    print("=" * 50)
    print("🤖 KRISHNA×CODE BOT - INITIALIZING")
    print("=" * 50)
    ...
    os.system('clear')
    print("🤖 KRISHNA×CODE BOT - STARTING")
    print("=" * 50)
    ...
    os.system('clear')
    print("🤖 KRISHNA×CODE BOT - CONNECTING")
    print("=" * 50)
    print("┌────────────────────────────────────┐")
    print("│ ██████████████████████████████████ │")
    print("└────────────────────────────────────┘")
